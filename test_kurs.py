...
perevirka("видача: інтро і тільки перший урок", metody == ["sendVideo", "sendMediaGroup"], str(metody))
perevirka("відео летить як video", any(m == "sendVideo" and p.get("video") == "V1" for m, p in VYKLYKY))
perevirka("текст після уроку", any("Це весь урок" in x for x in teksty()))
perevirka("інтро зі списком уроків", any("Готово, курс ваш" in x and "1. Урок 1" in x and "2. Урок 2" in x for x in teksty()))
perevirka("заголовок уроку 1 надісланий, уроку 2 ще ні", "Урок 1" in teksty() and "Урок 2" not in teksty())
...
perevirka("кнопка «Далі: Урок 2»", "Далі: Урок 2" in ostannia_klaviatura(1), str(ostannia_klaviatura(1)))
VYKLYKY.clear(); cb("les:k1:1", uid=1)
perevirka("урок 2 по кнопці: альбом з двох документів", [m for m, _ in VYKLYKY if m.startswith("send") and m != "sendMessage"] == ["sendMediaGroup"] and "Урок 2" in teksty())
perevirka("після останнього уроку курсу 1 веде в курс 2", "Далі: курс 2, колір" in ostannia_klaviatura(1), str(ostannia_klaviatura(1)))
VYKLYKY.clear(); cb("les:k1:1", uid=777)
perevirka("чужий не отримує урок", not [m for m, _ in VYKLYKY if m == "sendMediaGroup"] and any("не куплено" in x for x in teksty()))
VYKLYKY.clear(); msg("/moi", uid=1)
perevirka("/moi адміну: гайд і всі уроки", ostannia_klaviatura(1)[:2] == ["Гайд «Світло»", "Урок 1"], str(ostannia_klaviatura(1)))
VYKLYKY.clear(); msg("/moi", uid=777)
perevirka("/moi без покупок: магніт", "Забрати три схеми світла" in ostannia_klaviatura(777))
